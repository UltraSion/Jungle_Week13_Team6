...
void FParticleEmitterInstance::CheckEmitterFinished()
{
	...
	{
		const bool bNoContinuousSpawning = LODLevel->SpawnModule->Rate <= 0.0f;
...
float FParticleEmitterInstance::Tick_EmitterTimeSetup(float DeltaTime, UParticleLODLevel* InCurrentLODLevel)
{
	...
	bool bLooped = false;
	if (!RequiredModule->bUseLegacyEmitterTime)
	{
	...
	else
	{
		...
		if (EmitterDuration > 1.0e-6f)
		{
			EmitterTime = std::fmod(SecondsSinceCreation, EmitterDuration);
			bLooped = ((SecondsSinceCreation - (EmitterDuration * static_cast<float>(LoopCount))) >= EmitterDuration);
		}
	}
...
float FParticleEmitterInstance::Spawn(float DeltaTime)
{
	...
	if (SpriteTemplate->QualityLevelSpawnRateScale > 0.0f)
	{
		...
		if (LODLevel->SpawnModule)
		{
			...
			{
				SpawnRate =
					std::max<float>(0.0f, LODLevel->SpawnModule->Rate) *
					std::max<float>(0.0f, LODLevel->SpawnModule->RateScale);
			}
